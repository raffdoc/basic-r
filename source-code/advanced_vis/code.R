library(tidyverse)
library(countrycode)
source("/home/rafik/R_Programming/basic-r/source-code/advanced_vis/henrik.r")

df.fertility <- read_csv('https://ourworldindata.org/grapher/children-per-woman-UN.csv')
df.footprint <- read_csv('/home/rafik/R_Programming/basic-r/data/footprint.csv')
...
special_countries <- c("Luxembourg", "Aruba", "Australia", "Moldova")
df.tmp <- df.tmp |>
  mutate(sp_label = if_else(country %in% special_countries, country, ""))
...
gg <- df.tmp %>%
  ...
  #geom_label(aes(label = sp_label), nudge_x = 0.5, nudge_y = -0.2, alpha = 0.2) + 
